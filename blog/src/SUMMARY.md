[About](./about.md)

## Design History

- [Design History](./design/introduction/content.md)
    - [Why was Cedar created?](./design/why-cedar/content.md)
    - [Why doesn't Cedar have regexes and string operators?](./design/why-no-regex/content.md)
    - [Why doesn't Cedar support floating point numbers?](./design/why-no-float/content.md)
    - [Why doesn't Cedar allow wildcards in Entity Ids?](./design/why-no-entity-wildcards/content.md)
    - [Why does Cedar ignore policies that error?](./design/why-ignore-errors/content.md)
    - [Why do Cedar policies end with a semi-colon?](./design/why-semicolon/content.md)

- [Cedar Usage](./usage/introduction/content.md)
    - [How well do Cedar policies compress?](./usage/compression/content.md)

